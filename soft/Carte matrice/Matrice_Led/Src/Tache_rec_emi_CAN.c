...
//-------------------------------------------------------//
//
// Pour chaque source d'interruption possible:
//       - On recupere la trame CAN recue pour le buffer correspondant CAN_RX_FIFO0 ou CAN_RX_FIFO1 (voir librairie HAL bus CAN)
//       - On appel la procédure de traitement
//
// Fonction utilisée : TrtRecCAN(() + librairie HAL bus CAN
//
// Variable globale : hcan
//
...
//
//      PE : ptDataRec : pointeur sur les données CAN recues (voir description dans l'anayse SysML (diagramme d'activité "Gestion commande")
//           ptAppelImg : pointeur sur la structure de gestion des images (type t_AppImg dans "main.h")
//
//      PS : Aucun
//
// En fonction de la commande reçue dans la trame on effectue les actions suivantes
// (voir description sysML : diagramme d'activité "Gestion commande")
//
//    MAJ_LUMINOSITE : - on affecte la valeur du PWM avec le parametre correspondant. (voir librairie HAL)
//                     - on affecte le champ correspondant de la structure AppelImg avec le même paramètre
//
//    APP_IMG_SIMPLE : - On affecte le champ de la structure "MajImg" a VRAI
//					   - On affecte les champ "PageHaut" et "NumImage" avec les données reçues
//
//    APP_IMG_VOLET :  - On affecte les champs "TypeVolet" et "TempoVolet" de la structure "MajImg" avec les données reçues
//                     - On demarre le timer 1 sous IT (voir librairie HAL)
//                     - On affecte la variable globale "MemITTimer" à 1
//
//    MAJ_CLIGNOTTE :  - On affecte le champ "TempoCli" de la structure "MajImg" avec la donnée reçue
//
//    APP_2_IMAGE   :  - On affecte le champ "Tempo2Img" de la structure "MajImg" avec la donnée reçue
//
// Variable globale : MemITTimer, htim3
//
void GesCmdImage(uint8_t *ptDataRec, t_AppImg *ptAppelImg)
{
uint8_t Param;
...
//      PS : Aucun
//
// - Initialisation des filtres CAN: On filtre sur l'ID en 32 bits pour la reception des messages de commande
//                                   On positionne le numero de debut de la bank de filtre à 14 (milieu entre 0 et 27)
//                                   Pour le filtre 1 (bank 0) : on doit accepter les ID "ID_REC_IMG_BMP" et "ID_REC_IMG_CMD"
//                                   Pour le filtre 2 (bank 1) : on doit accepter les ID "ID_MSG_CONFIG_EEP" à "ID_MSG_CONFIG_EEP+3"
// - Demarrge du bus CAN (voir librairie HAL bus CAN)
// - Initialisation des notifications sous IT pour FIFO0 et FIFO1
//
//
