...
#define ADR_CAT24M01_PB  0b10100000
#define ADR_CAT24M01_PH  0b10100010
...
{
unsigned short EcrOK = VRAI;
//unsigned short Res = VRAI;
unsigned short i = 0;
         
         //------- debut d'echange -------//
         sspcon2.SEN = 1;
         delay_us(ATTENTE_POLLING);
         //----- ecriture adresse I2C ----//
         if(PageHaute == VRAI)                                 //si Page = Haute alors
         {
               sspbuf = ADR_CAT24M01_PH;                      //l'adresse I2C prend la valeur 0b10100010
         }
         else
         {
               sspbuf = ADR_CAT24M01_PB;                       //l'adresse I2C prend la valeur 0b10100000
         }
         delay_us(ATTENTE_POLLING);
          //-- vérification de l'acquittement --//
          if(sspcon2.ACKSTAT == 0)                             //si ACK = OK
          {
               //---écriture adresse interne haute de la mémoire ---//
               sspbuf = hi(AdresseInt);
               delay_us(ATTENTE_POLLING);
               //-- vérification de l'acquittement --//
               if(sspcon2.ACKSTAT == 0)                             //si ACK = OK
               {
                   //---écriture adresse interne basse de la mémoire ---//
                   sspbuf = lo(AdresseInt);
                   delay_us(ATTENTE_POLLING);

                   if(sspcon2.ACKSTAT == 0)                             //si ACK = OK
                   {
                        while ((NbData != 0) && (EcrOK == VRAI))
                        {
...
unsigned short AdrCAT = ADR_CAT24M01;
         ...
         delay_us(ATTENTE_POLLING);
          ...
          if(sspcon2.ACKSTAT == 0)                             //si ACK = OK
          {
               ...
               if(sspcon2.ACKSTAT == 0)                             //si ACK = OK
               {
                   //---ecriture adresse interne basse de la mémoire ---//
                   sspbuf = lo(AdresseInt);
                   delay_us(ATTENTE_POLLING);
               }
               else
...
unsigned short i = 0;
         ...
         sspcon2.SEN = 1;
         delay_us(ATTENTE_POLLING);
         sspbuf = ADR_CAT24M01_PB+1;    //---- +1 car on est en lecture -----//
         delay_us(ATTENTE_POLLING);
         while(i != NbByte2Read-1)
         {
              //--pasage en mode lecture--//
              sspcon2.RCEN = 1;
              delay_us(ATTENTE_POLLING);
         ...
         sspcon2.RCEN = 1;            //la derniere donnée lue doit etre suivi
         delay_us(ATTENTE_POLLING);   //d'un NACK (indique a la memoire la fin de lecture
         ptBufferRead[i] = sspbuf;
         sspcon2.ACKDT = 1;
         sspcon2.ACKEN = 1;
         delay_us(ATTENTE_POLLING);
